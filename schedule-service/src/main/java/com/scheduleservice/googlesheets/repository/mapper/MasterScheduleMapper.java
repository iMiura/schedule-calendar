...
public interface MasterScheduleMapper extends BaseMapper<MasterScheduleEntity> {
    ...
    @Select({
        ...
        "  from (select @lastDay := date_add((select case when unix_timestamp(m.master_deadline) - unix_timestamp(CURRENT_DATE()) >= 0 THEN m.master_deadline ELSE CURRENT_DATE() END ",
